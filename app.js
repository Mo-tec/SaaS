...
async function testSupabaseConnection() {
  const connectionEl = $('devSyncConnection');
  const supabase = getSupabaseClient();

  if (!supabase) {
    if (connectionEl) connectionEl.textContent = 'Imezimwa';
    showNotification('Supabase bado haijawekwa.');
    return false;
  }

  if (connectionEl) connectionEl.textContent = 'Inajaribu...';
  try {
    const { data: sessionData, error: sessionError } = await supabase.auth.getSession();
    if (sessionError) throw sessionError;

    const statusText = sessionData?.session ? 'Imeunganishwa (imeningia)' : 'Imeunganishwa (anon tayari)';
    if (connectionEl) connectionEl.textContent = statusText;
    updateSyncStatusDisplay(statusText);
    showNotification('Muunganisho wa Supabase umefanikiwa.');
    return true;
  } catch (error) {
    const message = error?.message || 'Unknown error';
    if (connectionEl) connectionEl.textContent = 'Kosa';
    updateSyncStatusDisplay(`Kosa la Supabase: ${message}`);
    showNotification(`Muunganisho wa Supabase umefaili: ${message}`);
